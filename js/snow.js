(function (global, factory) {
  ...
  function Snow(option) {
    ...
    if (option) {
      ...
      if (typeof option.randombase === "number")
        this.randombase = option.randombase;
      if (typeof option.stayTime === "number") this.stayTime = option.stayTime;
      if (typeof option.hideDuration === "number")
        this.hideDuration = option.hideDuration;
      if (typeof option.shadowBlur === "number")
        this.shadowBlur = option.shadowBlur;
      if (typeof option.shadowSize === "number")
        this.shadowSize = option.shadowSize;
      if (typeof option.zIndex === "number") this.zIndex = option.zIndex;

      if (typeof option.offset === "boolean") this.isOffset = option.offset;
      if (typeof option.isOffsetOnTouch === "boolean")
        this.isOffsetOnTouch = option.isOffsetOnTouch;
      ...
      if (typeof option.reversionOffset === "boolean")
        this.reversionOffset = option.reversionOffset;
      if (typeof option.constrain === "boolean")
        this.constrain = option.constrain;
      if (typeof option.randomColor === "boolean")
        this.randomColor = option.randomColor;

      if (typeof option.imgurl === "string") this.img = option.imgurl;
      if (typeof option.color === "string") this.color = option.color;
      if (typeof option.cusShadowColor === "string")
        this.cusShadowColor = option.cusShadowColor;
  ...
  Snow.prototype.setOffset = function (xPos) {
    ...
    this.createSnowOffset =
      xPos - 0.5 > 0 ? -this.clientWidth * 0.5 : this.clientWidth * 0.5;
  };
  ...
  Snow.prototype.createSnow = function () {
    ...
    var timer = setInterval(function () {
      ...
      for (var i = 0; i < parseInt(Math.random() * _this.num) + 1; i++) {
        ...
        var size =
          parseInt(
            Math.random() * (_this.img ? _this.maxImgSize : _this.maxSize)
          ) + 6;
        ...
        if (_this.img) {
          ...
          if (typeof _this.img === "string") {
            ...
            snowFlake.setAttribute(
              "src",
              _this.img[parseInt(Math.random() * _this.imgLength)]
            );
          }
        }
        ...
        snowFlake.style.top =
          -size - (_this.shadow ? _this.shadowSize : 0) + "px";
        snowFlake.style.left =
          parseInt(Math.random() * _this.clientWidth) +
          _this.createSnowOffset +
          "px";
        ...
        if (!_this.img) {
          ...
          if (_this.randomColor) {
            snowFlake.style.backgroundColor =
              "#" +
              (
                "00000" + ((Math.random() * 0x1000000) << 0).toString(16)
              ).substr(-6);
            _this.shadowColor = snowFlake.style.backgroundColor;
          } else if (typeof _this.color === "string") {
            // 判断传入的颜色是不是数组
            snowFlake.style.backgroundColor = _this.color;
          } else {
            snowFlake.style.backgroundColor =
              _this.color[parseInt(Math.random() * _this.colorLength)];
            _this.shadowColor = snowFlake.style.backgroundColor;
          }
        }

        _this.img || !_this.shadow
          ? ""
  ...
  Snow.prototype.move = function (ele, thisOffset) {
    ...
    var timer = setInterval(function () {
      ...
      // 雪花的左右偏移计算
      ele.style.left =
        parseFloat(ele.style.left) + _this.offset + thisOffset + "px";

      // 计算雪花停止移动的位置
      var destoryDistence = _this.stay
        ? _this.clientHeight - parseInt(ele.style.height)
        : _this.clientHeight;
